...
export function CardDetailModal({ card, entry, onClose, onUpdateQuantity, onRemove }: CardDetailModalProps) {
  ...
  return (
    <div
      className="fixed inset-0 bg-black/50 flex items-end z-50 transition-opacity duration-200"
      style={{ opacity: visible ? 1 : 0 }}
      onClick={handleClose}
    >
      <div
        className="w-full bg-white rounded-t-2xl p-5 pb-8 transition-transform duration-280 ease-out"
        style={{ transform: `translateY(${visible ? dragY : '100%'}px)`, transitionProperty: dragY > 0 ? 'none' : 'transform' }}
      ...
      >
        ...
        <div className="flex gap-4 mb-5">
          ...
          <div className="flex-1 flex flex-col gap-1.5">
            <h2 className="font-bold text-lg text-slate-900 leading-tight">{card.name}</h2>
            <p className="text-sm text-slate-400">{card.id}</p>
            {card.rarity && (
        ...
        {entry && onUpdateQuantity && onRemove ? (
          <>
          ...
          <div className="py-3 text-center text-sm text-slate-400 bg-slate-50 rounded-xl">
            Non possédée
          </div>
        )}
      </div>
